...
//					"<>Identifier: "+useCaseDiag.toString());
			...
			String dataFileName = packageName.concat("_" + modelDiagName).concat("_UcDiagram.json"); // packageName_ModelName_
...
//						String ownerElementName = req.getOwner().getName();
				...
				} else if (eachModelElement.getMetaClass().equals("AssociationEnd")) {
...
//		int count = allNested.getCount();
		...
		if (eachPkgElement.getUserDefinedMetaClass().equalsIgnoreCase("Statechart")) {
			...
			// Each Model Element
			String allReqElementData = ""; // model level requirements data file
			String dataFileName = packageName.concat("_" + modelDiagName).concat("_StmDiagram.json"); // packageName_ModelName_
...
//						IRPEvent event = IRPEvent
	...
	public void printActivityDiagram(IRPModelElement eachPkgElement) throws IOException {
		if (eachPkgElement.getUserDefinedMetaClass().equalsIgnoreCase("ActivityDiagram")) {
			...
			// Each Model Element
			String allReqElementData = ""; // model level requirements data file
			String dataFileName = packageName.concat("_" + modelDiagName).concat("_ActDiagram.json"); // packageName_ModelName_
...
//						reqElementProperties.put("description", req.getDescription());// requirementDesc
					...
					/*
					 *  ------ Ignoring this relationship block for now. Because we need to identify proper source block for pin
					// create req.node - req.diagramNode (model) relationship
					JSONObject reqRelation = new JSONObject();
					reqRelation.put("id", "");
					reqRelation.put("type", "relationship");
					...
					JSONObject reqRelationStart = new JSONObject();
					reqRelationStart.put("id", reqElementOwnerIdentifier); // Source State
	...
	public void printIBD() throws IOException {
		...
		for (IRPClass classItem : classes) {
			...
			for (int j = 1; j <= amountOfPorts; j++) {
				...
				JSONObject reqElement = new JSONObject();
				reqElement.put("id", port.toString());
				reqElement.put("type", "node"); //Lifeline

				JSONArray reqElementlabelsArray = new JSONArray();
				reqElementlabelsArray.add("port");
				reqElement.put("labels", reqElementlabelsArray);

				JSONObject reqElementProperties = new JSONObject();
				reqElementProperties.put("name", port.getName());
				reqElementProperties.put("type", port.getUserDefinedMetaClass());
				reqElementProperties.put("genericType", port.getMetaClass());

				reqElement.put("properties", reqElementProperties);

				allReqElementData = allReqElementData.concat(reqElement.toJSONString()).concat("\n");
				System.out.println("Port Node ~~> " + reqElement.toJSONString());
				...
				JSONObject reqRelationProperties = new JSONObject();
				reqRelationProperties.put("type", port.getMetaClass()); // State

				JSONObject reqRelationStart = new JSONObject();
				reqRelationStart.put("id", port.toString()); // Source

				JSONObject reqRelationEnd = new JSONObject();
				reqRelationEnd.put("id", port.getOwner().toString()); // Target
				...
				allReqElementData = allReqElementData.concat(reqRelation.toJSONString()).concat("\n");
				System.out.println("Port Relation --> " + reqRelation.toJSONString());
			}
			...
			for (int j = 1; j <= amountOfFlows; j++) {
				...
				reqRelation.put("id", "");
				reqRelation.put("type", "relationship");
				reqRelation.put("label", "HAS_FLOWS_TO");

				JSONObject reqRelationProperties = new JSONObject(); 
				reqRelationProperties.put("name", flow.getName());
				reqRelationProperties.put("type", flow.getMetaClass());

				JSONObject reqRelationStart = new JSONObject();
				reqRelationStart.put("id", source_element); // Source

				JSONObject reqRelationEnd = new JSONObject();
				reqRelationEnd.put("id", target_element); // Target
				...
				allReqElementData = allReqElementData.concat(reqRelation.toJSONString()).concat("\n");
				System.out.println("Flow Relation --> " + reqRelation.toJSONString());
			}
...
//		int count = allNested.getCount();
		...
		if (eachPkgElement.getUserDefinedMetaClass().equalsIgnoreCase("Block Definition Diagram")) {
			...
			// Each Model Element
			String allReqElementData = ""; // model level requirements data file
			String dataFileName = packageName.concat("_" + modelDiagName).concat("_BddDiagram.json"); // packageName_ModelName_
...
//						reqElementProperties.put("description", req.getDescription());// requirementDesc
					...
					IRPCollection relations = req.getRelations();
...
//							System.out.println("Relation type: " + relation.getRelationType());
	...
	public void printElementsSeqDiagram(IRPSequenceDiagram mod) {
		...
		// Each Model Element
		String allReqElementData = ""; // model level requirements data file
		String dataFileName = packageName.concat("_" + mod.getName()).concat("_SeqDiagram.json"); // packageName_ModelName_
